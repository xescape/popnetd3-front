...
var edgeAttrs = {
  ...
  "stroke-width": function (d) {
    return translateEdge(d.width) + "px";
  },
};
...
svg = svg.append("svg").attr("id", "popnet");
...
document
  ...
  .addEventListener("click", save_network, false);

document
  .getElementById("button-linear")
  .addEventListener("click", getLinear, false);
...
document
  ...
  .addEventListener("keypress", launchByKey, false);

document
  .getElementById("example")
  .addEventListener("click", launch_example, false);
...
function draw(data) {
  ...
  simulation.on("tick", function () {
    ...
    edges
      .attr("x1", function (d) {
        return d.source.x + nodeRadius + borderWidth;
      })
      .attr("y1", function (d) {
        return d.source.y + nodeRadius + borderWidth * 1.5;
      })
      .attr("x2", function (d) {
        return d.target.x + nodeRadius + borderWidth;
      })
      .attr("y2", function (d) {
        return d.target.y + nodeRadius + borderWidth * 1.5;
      });
  ...
  nodes.call(drag).each(getChr);
  //		.call(getChr(d))
  //		.append(function(d){return getChr(d);})
  //	nodes.each(function(d){appendSVG(d)});

  //bind nodes and edges
  simulation.nodes(data.nodes).force("link").links(data.edges);

  //zoom and pan behavior
  svg.call(zoom).on("click", null);
  ...
  function groupForce(sim, container) {
    ...
    var e = container.selectAll(".glink").data(glinks).enter();
    //				.append('line')
    //				.attr('class' , 'glink');
...
function updateChrs(n) {
  ...
  var html = _.range(n)
    .map(function (val) {
      val = val + 1;
      return (
        '<li class="mdl-menu__item" data-val="CHR' +
        val +
        '">Chromosome ' +
        val +
        "</li>\n"
...
function updateEdgeCutoff() {
  ...
  var html = Object.keys(edge_weights)
    .map(function (val) {
      return (
        '<li class="mdl-menu__item" data-val="' + val + '">' + val + "</li>\n"
      );
    })
    .join("");
...
function translateEdge(p) {
  var a = edge_weight_keys.map(function (a) {
    return Math.abs(parseFloat(a) - p);
  });
...
function appendSVG(node) {
  ...
  var svgtext = $.get(svgpath, function () {
    svgdom = domparser
      .parseFromString(svgtext.responseText, "image/svg+xml")
      .querySelectorAll("[name='" + node.name + "']")[0];
...
function getChr(node) {
  ...
  svg
    .attr(
      "height",
      (nodeRadius * 2 + borderWidth * 4) * scale + +labelAttrs["font-size"]
    )
    .attr("width", (nodeRadius * 2 + borderWidth * 4) * scale);

  var g = svg.append("g").attr("transform", "scale(" + 1 / scale + ")");
...
function paintChr(node) {
  ...
  var svg = d3
    ...
    .attr(
      "transform",
      "translate(" + nodeRadius * 1.1 + "," + nodeRadius * 1.1 + ")"
    );
  var arc = d3
    .arc()
  ...
  var circle = svg.append("circle").attr("stroke", function (d) {
    return colorTable[node.name];
  });
  ...
  label = svg.append("svg:text").text(node.name);
...
function groupCircle(simulation, reset = false) {
  ...
  function calculatePositions(g, n) {
    ...
    function placeNodes(g, n, i, k) {
      var r = Math.max(
        (n * (nodeRadius * 2 + borderWidth * 4)) / (2 * Math.PI),
        Math.max(
          2 * (nodeRadius + 20),
          (k * (i + 1) * (nodeRadius * 2 + borderWidth * 4)) / (2 * Math.PI)
        )
      );
    ...
    var results = rings.map(function (n, i) {
      return placeNodes(g, n, i, k);
    });
...
function save(to_save, name) {
  ...
  var x_offset =
    (document.getElementById("container").clientWidth - base_width) / 2;
  ...
  //add content
  SVGtoPDF(doc, target, x_offset, y_offset, {
    useCSS: true,
    width: width,
    height: height,
  });
...
function getLinear() {
  ...
  if (document.getElementById("linear")) {
    ...
    var file = input.match(/[/](\w+?).json/)[1];
    var e = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    var svg = d3.select(this).append(function (d) {
      return e;
    });
    ...
    var width_attr =
      length + padding[1] + padding[3] + front + margin.left + margin.right;
    height_attr =
      (height + padding[0] + padding[2]) * (node_list.length + 1) +
      margin.top +
      margin.bottom;
...
function removeLinearPanel() {
  document
    ...
    .removeChild(document.getElementById("linear"));
}

function save_painting() {
  imgURI = document
    .getElementById("chr_painting")
