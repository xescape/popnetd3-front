/**
	Contains functions for running a popnet job
 */
...
var cpUpload = upload.fields([
  ...
  { name: "ival", maxCount: 1 },
  { name: "pival", maxCount: 1 },
  { name: "sl", maxCount: 1 },
    ...
    exec(
      ...
      function (error, stdout, stderr) {
        if (!error) {
          ...
          fs.rename(oldpath, newpath, function () {
            fs.rename(oldheatpath, newheatpath, function () {
              transporter.sendMail(
                makeEmail(config.email, id, true, newheatpath),
          ...
          transporter.sendMail(
            makeEmail(config.email, id, false, errorpath),
            function (err, info) {
              if (err) {
                console.log(`emailing got this error:\n${err}
						code:${err.code}
          ...
          transporter.sendMail(
            makeEmail(selfemail, id, false, errorpath),
            function (err, info) {
              if (err) {
                console.log(`emailing got this error:\n${err}
						code:${err.code}
