...
function getLinChr(req, res) {
  ...
  svg2png(drawLinear(params))
    .then(function (buffer) {
      ...
      res.sendFile(params["outpath"], { root: __dirname }, function (err) {
        if (err) console.log("linear  sendfile error, " + err);
      });
    })
    .catch(function (err) {
      console.log("svg2png error " + err);
    });
}
...
function getPainting(req, res) {
  ...
  params["outpath"] =
    "/c/" + params["file"] + "/" + params["id"] + "_" + params["chr"] + ".png";
...
function paintChr(node, colorTable, outpath, mode = "PNG") {
  ...
  svg = d3n
    ...
    .attr(
      "height",
      (nodeRadius + borderWidth * 2) * 2 + settings.labelAttrs["font-size"]
    )
    .attr("name", node.name)
    .append("g")
    .attr(
...
function drawLinear(params) {
  ...
  var band = d3
    .scaleBand()
    .rangeRound([0, (height + padding[0]) * nodes.length])
    .paddingInner(0.05)
    .align(0.1);
  ...
  scale
    .domain([
      d3.max(nodes, function (d) {
        return d.lengths.reduce(function (t, d) {
          return t + parseInt(d);
        }, 0);
      }),
      0,
  ...
  var selection = svg.selectAll(".nodes").data(nodes).enter().append("g");

  var stacks = selection
    ...
    .attr("x", function (d, i) {
      if (i === 0) return 0;
        ...
        return (
          parseFloat(prev.getAttribute("x")) +
          parseFloat(prev.getAttribute("width"))
        );
      }
